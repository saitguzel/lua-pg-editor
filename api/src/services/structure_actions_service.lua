-- Yapı ogesi eylemleri (codd structure_actions): kolon / index / constraint / trigger yeniden adlandir ve sil.
...
-- kind → { rename = fn(tbl, schema, item, new), drop = fn(tbl, schema, item) } ; tbl = "şema"."tablo"
local SQL = {
  column = {
...
local function map_err(err)
  ...
  if code == "42P01" or code == "42703" or code == "42704" then
    return errors.new("OBJECT_NOT_FOUND", "Oge bulunamadı", { sqlstate = code, db_message = msg })
  end
