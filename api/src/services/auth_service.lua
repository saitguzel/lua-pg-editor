...
function _M.verify_reset_token(token)
  local hash = jwt.sha256_hex(token)
  ...
  if row.expires_at then
    -- use now() compare via DB already? But double check via query for expiry: token valid only if expires_at > now()
    local valid, verr = query.query_one(
      [[SELECT 1 AS ok FROM password_reset_tokens WHERE token_hash = $1 AND used_at IS NULL AND expires_at > now() LIMIT 1]],
      hash
    )
