...
OUT=/backups/$TAG/pgeditor-$TS.dump
mkdir -p "/backups/$TAG"

PGPASSWORD=$(cat /run/secrets/db_password 2>/dev/null || echo "${PGPASSWORD:-}")
export PGPASSWORD
pg_dump -h "${PGHOST:-postgres}" -U "${PGUSER:-pgeditor}" -d "${PGDATABASE:-pgeditor}" -Fc -Z 6 -f "$OUT"
...
keep() {
  ls -1t "$1"/pgeditor-* 2>/dev/null | tail -n +$(($2 + 1)) | xargs -r rm -f
}
