...
function _M.render(state, dispatch)
  ...
  return dom.div({ class = "max-w-4xl space-y-4 sm:space-y-6" },
    dom.h1({ class = "text-xl sm:text-2xl font-bold flex items-center gap-2", tabindex = "-1" },
      icons.get("settings", "w-6 h-6 text-[var(--primary)] shrink-0"), "Ayarlar"),
    dom.section({ class = "space-y-3 p-3 sm:p-4 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-elev)]" },
      dom.h2({ class = "font-semibold flex items-center gap-2 text-sm sm:text-base" }, icons.get("settings", "w-4 h-4 text-[var(--primary)]"), "Tema"),
    ...
    dom.section({ class = "space-y-3 p-3 sm:p-4 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-elev)]" },
      dom.h2({ class = "font-semibold flex items-center gap-2 text-sm sm:text-base" }, icons.get("table", "w-4 h-4 text-[var(--primary)]"), "Tablo varsayılanları"),
      dom.div({ class = "grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4" },
        dom.div({},
    ...
    dom.section({ class = "space-y-2 p-3 sm:p-4 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-elev)]" },
      dom.h2({ class = "font-semibold flex items-center gap-2 text-sm sm:text-base" }, icons.get("eye", "w-4 h-4 text-[var(--primary)]"), "Görünüm"),
    ...
    dom.section({ class = "p-3 sm:p-4 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg)]" },
      dom.h2({ class = "font-semibold mb-2 flex items-center gap-2 text-sm sm:text-base" }, icons.get("info", "w-4 h-4 text-[var(--primary)]"), "Sistem"),
      dom.dl({ class = "grid grid-cols-1 sm:grid-cols-[10rem_1fr] gap-y-1 text-sm" },
