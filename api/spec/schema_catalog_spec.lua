...
describe("target_schema_repo kategori SQL", function()
  ...
  it("q/limit/offset SQL'e girer; offset 0 yazilmaz (parametreli LIMIT/OFFSET faz-31)", function()
    local sql = repo.category_sql("tables", true, 200, 0)
    assert.truthy(sql:find("ILIKE $2 ESCAPE '\\'", 1, true))
    assert.truthy(sql:find("LIMIT $3", 1, true))
    assert.is_nil(sql:find("OFFSET", 1, true))
    assert.truthy(repo.category_sql("views", false, 10, 40):find("LIMIT $2 OFFSET $3", 1, true))
    assert.truthy(repo.category_sql("tables", false, 10, nil):find("LIMIT $2", 1, true))
    assert.is_nil(repo.category_sql("tables", false, nil, nil):find("LIMIT", 1, true))
  end)
  ...
  it("tip kategorisi dizi ve tablo satır tiplerini haric tutar", function()
