...
function pagination.render(meta, on_change)
  ...
  return dom.nav({ ["aria-label"] = "Sayfalama", class = "flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-2 flex-wrap py-2" },
    dom.div({ class = "flex items-center gap-1 justify-center sm:justify-start" },
      ...
      dom.span({ class = "text-sm text-[var(--fg-muted)] px-2 py-1 rounded-full bg-[var(--bg-elev)] border border-[var(--border)]", ["aria-current"] = "page" },
        "Sayfa " .. page .. " / " .. last),
    ...
    dom.div({ class = "flex items-center gap-2 justify-center sm:justify-end flex-wrap" },
      dom.span({ class = "text-xs text-[var(--fg-muted)] whitespace-nowrap", role = "status" },
        first_row .. "-" .. last_row .. " / " .. total .. " satır"),
      dom.label({ class = "text-xs text-[var(--fg-muted)] flex items-center gap-1 whitespace-nowrap" }, "Satır/sayfa",
