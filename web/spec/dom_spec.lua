...
describe("dom.patch ayni vnode", function()
  it("ayni vnode ile tekrar patch dinleyiciyi korur (modal.show icerigi)", function()
    local fake = require("helper")
    local dom = require("dom")
    local clicks = 0
    local root = fake.dom.create("div")
    local v = dom.div({}, dom.button({ onclick = function() clicks = clicks + 1 end }, "x"))
    local tree = dom.patch(root, nil, dom.div({}, v))
    tree = dom.patch(root, tree, dom.div({}, v))
    tree = dom.patch(root, tree, dom.div({}, v)) -- eskiden burada _box nil hatasi
    fake.fire(v.children[1]._h, "click")
