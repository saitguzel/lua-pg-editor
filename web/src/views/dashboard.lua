...
function _M.render(state, dispatch)
  ...
  if conns.status == "error" then
    return dom.div({ class = "p-8 text-center", role = "alert" },
      ...
      dom.button({ class = "px-4 py-2 border rounded", onclick = function() app.dispatch({ type = "CONNECTIONS_REQUESTED" }) end }, "Tekrar dene"))
  end
  ...
  return dom.div({ class = "space-y-4" },
    ...
    empty and require("views.layout").empty_state({
      ...
      dom.div({ class = "p-4 border border-[var(--border)] rounded-[var(--radius)] bg-[var(--bg-elev)]" },
    ...
    dom.div({ class = "flex gap-2 mt-4" },
      dom.a({ href = "#/connections", class = "px-4 py-2 rounded bg-[var(--primary)] text-[var(--primary-fg)]" }, "Bağlantılara git"),
      dom.a({ href = "#/query", class = "px-4 py-2 rounded border border-[var(--border)]" }, "Sorgu çalıştır")))
end
