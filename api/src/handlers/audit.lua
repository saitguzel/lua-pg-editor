-- Audit okuma endpoint'leri: liste, detay, istatistik, CSV export (pg-editor F12)
...
-- ISO-8601 (validation.datetime'dan gecmis) -> epoch saniye (UTC)
...
-- Sorgu parametrelerini dogrular; from/to varsayilani son 7 gun, aralik en fazla 90 gun
local function parse_audit_query()
  local raw = ngx.req.get_uri_args()
  -- search alias -> q (Faz12: search ILIKE payload)
  ...
  if from_t >= to_t then
    return nil, errors.validation({ from = { "from, to'dan once olmali" } })
  end
  if to_t - from_t > MAX_RANGE_DAYS * DAY then
    return nil, errors.validation({ from = { "aralik en fazla " .. MAX_RANGE_DAYS .. " gun" } })
  end
