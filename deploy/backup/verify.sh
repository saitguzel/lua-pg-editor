...
# Kullanım (repo kökünden): sh deploy/backup/verify.sh   [BACKUP_VOLUME=pg-prod_backups]
# Şifreli yedek: BACKUP_AGE_IDENTITY=/yol/age.key BACKUP_IMAGE=pg-backup:<TAG>
set -eu

NAME=pg-restore-drill-$$
VOL=${BACKUP_VOLUME:-pg-prod_backups}
...
LATEST=$(docker run --rm -v "$VOL":/backups:ro alpine sh -c \
  'ls -1t /backups/daily/pgeditor-* /backups/pre-deploy/pgeditor-* 2>/dev/null | head -n1')
[ -n "$LATEST" ] || { echo "HATA: yedek yok" >&2; exit 1; }
echo "==> tatbikat: $LATEST"

trap 'docker rm -f "$NAME" >/dev/null 2>&1 || true' EXIT
docker run -d --name "$NAME" -e POSTGRES_PASSWORD=drill -e POSTGRES_USER=pgeditor -e POSTGRES_DB=pgeditor \
...
i=0; until docker exec "$NAME" pg_isready -U pgeditor -d pgeditor >/dev/null 2>&1; do
