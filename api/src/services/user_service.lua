...
function _M.list(_, q)
  ...
  local filters = { q = q.q or q.search, role = q.role, is_active = is_active }
...
function _M.create(_, input)
  ...
  local fields = {
    ...
    role = input.role or "editor",
    is_active = input.is_active,
  }
  if fields.is_active == nil then fields.is_active = true end
  -- nullable full_name: cjson.null -> nil
  if fields.full_name == require("cjson.safe").null then fields.full_name = nil end
...
function _M.update(identity, id, input)
  ...
  local ok, err = query.with_transaction(function()
    ...
    -- handle nullable full_name cjson.null
    if fields.full_name == require("cjson.safe").null then fields.full_name = nil end
