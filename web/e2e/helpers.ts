...
export const API = process.env.API_BASE_URL ?? "http://localhost:28180/api/v1";
export const ADMIN = { email: "admin@pgeditor.local", password: "Admin123!" };
export const EDITOR = { email: "editor@pgeditor.local", password: "Editor123!" };
...
export function unique_email(prefix = "e2e"): string {
  return `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}@pgeditor.local`;
}
export function unique_name(prefix = "e2e"): string {
...
export async function login(page: Page, email: string, password: string): Promise<void> {
  ...
  // dashboard heading
  await expect(page.getByRole("heading", { name: /Gösterge|Pano/ })).toBeVisible({ timeout: 15_000 });
}
...
export async function create_user(request: APIRequestContext, role: "admin" | "editor" = "editor"): Promise<{ email: string; password: string }> {
...
// Bağlantı oluştur (API ile, UI testleri için)
export async function create_connection(request: APIRequestContext, token: string, overrides: Record<string, unknown> = {}) {
  const payload = {
