...
describe("router.parse", function()
  it("path ve query ayırır", function()
    local r = router.parse("#/connections?search=pg&page=2")
    assert.equal("/connections", r.path)
    assert.equal("pg", r.query.search)
    assert.equal("2", r.query.page)
  end)

  it("parametreli route eşler (#/browse/:schema/:table)", function()
    local r = router.parse("#/browse/public/customers")
    assert.equal("browse", r.name)
    assert.equal("public", r.params.schema)
  end)

  it("URL-encoded query çözer", function()
    local r = router.parse("#/connections?q=fatura%20%C3%B6de")
  ...
  local cases = {
    ...
    { "#/connections", "connections" }, { "#/query", "query" }, { "#/browse/public/customers", "browse" },
    { "#/structure/public/customers", "structure" },
    { "#/users", "users" }, { "#/rbac", "rbac" }, { "#/audit?action=x", "audit" },
    { "#/profile", "profile" }, { "#/browse/a/b/c", "not_found" }, { "#/auditX", "not_found" },
  }
...
describe("router.guard", function()
  local auth_authenticated = { status = "authenticated", permissions = { ["connections.list"] = true } }
  ...
  it("unknown durumda bekler (nil)", function()
    local route = router.parse("#/connections")
  ...
  it("oturumsuz korumalı sayfa → login + next", function()
    ...
    assert.truthy(redirect:find("next=%23%2Fconnections", 1, true))
  end)

  it("editor page_key izni yoksa forbidden", function()
...
describe("router.safe_next", function()
  it("#/ ile başlayan kabul", function()
    assert.equal("#/connections", router.safe_next("#/connections"))
  end)
