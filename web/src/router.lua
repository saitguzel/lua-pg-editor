-- F17: Hash router + auth/RBAC guard. parse ve guard saf fonksiyonlardır.
-- pg-editor uyarlaması: connections / query / browse / structure / admin.

local router = {}

-- Route tablosu (kanonik, faz-17). Sıra önemli: sabit segmentler parametreliden önce.
...
local ROUTES = {
  ...
  { hash = "#/connections", name = "connections", view = "views.connections", auth = true, page_key = "connections.list" },
  { hash = "#/query", name = "query", view = "views.query_editor", auth = true, page_key = "query.execute" },
  { hash = "#/query/history", name = "query_history", view = "views.query_history", auth = true, page_key = "query.history" },
  { hash = "#/browse/:schema/:table", name = "browse", view = "views.table_browser", auth = true, page_key = "table.browser" },
  { hash = "#/structure/:schema/:name", name = "structure", view = "views.structure", auth = true, page_key = "structure.view" },
...
-- "#/connections/abc" → { "#", "connections", "abc" }
...
-- Saf: "#/connections/abc?x=1&y=iki%20kelime" → { path, name, params, query, raw, route }
