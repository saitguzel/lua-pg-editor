...
# --- 2. Lua bundle'ları: admin view'ları ayrı (editor hiç indirmez — F17 #7) ---
...
if [ "$MODE" = "production" ]; then
  ...
  DEMO='{ { label = "Yönetici", email = "admin@pgeditor.local", password = "Admin123!" },
    { label = "Editör", email = "editor@pgeditor.local", password = "Editor123!" } }'
fi
printf -- '-- build-wasm.sh tarafından üretilir\nreturn { mode = "%s", demo = %s }\n' "$MODE" "$DEMO" > "$OUT/build_info.lua"
node scripts/bundle-lua.mjs $MIN --exclude "$ADMIN_VIEWS" src= ../shared/src=pg_shared. \
