...
local function recover_once(err)
  ...
  local r = recovering[key]
  if r then
    local co = coroutine.running() -- 5.4: (co, ismain) — yalnizca ilki
    table.insert(r.waiters, co)
    return coroutine.yield()
  end
  r = { waiters = {} }
  recovering[key] = r
  local ok = cfg.on_recoverable(err) == true
  recovering[key] = nil
  for _, co in ipairs(r.waiters) do
    local res, e = coroutine.resume(co, ok)
    if not res then js.log("error", tostring(e)) end
  end
  return ok
end

local call
call = function(method, path, query, body, retried)
  ...
  if RECOVERABLE[err.code] and not retried and cfg.on_recoverable and recover_once(err) then
...
function api.delete(path, body) return call("DELETE", path, nil, body) end

-- CSV export: blob indirme (Authorization'ı JS tarafı ekler). body verilirse POST (JSON).
-- Coroutine içinde bitene kadar bekler; sunucu hatası normalize edilmiş API hatası olarak döner.
function api.download(path, filename, query, body)
  local tokens = cfg.get_tokens()
  local err, err_body = await(function(cb)
    js.http.download(cfg.base .. path .. encode_query(query), tokens and tokens.access_token or nil, filename, cb,
      body ~= nil and json.encode(body) or nil)
  end)
  if err then
    if err_body and err_body ~= "" then return nil, normalize({ text = err_body }, tonumber(err:match("%d+"))) end
    return nil, client_error("NETWORK_ERROR", tostring(err))
  end
  return true
end
