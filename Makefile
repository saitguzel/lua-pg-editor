...
TOOLS_IMAGE ?= pg-editor-api:latest
...
db.psql:
	docker compose exec postgres psql -U $${DB_USER:-pgeditor} $${DB_NAME:-pgeditor}
...
test.integration: up.e2e
	$(IT) busted --run=integration
	@echo "Not: yigin test modunda (tmpfs DB). Gelistirme yiginina donmek icin: make up"
...
rockspecs:
	$(TOOLS) luarocks make rockspecs/pg-shared-0.1.0-1.rockspec --tree /tmp/rocks
