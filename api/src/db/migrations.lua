-- Migration runner: dosyaları sırayla uygular, advisory lock ile korunur (pgeditor)
-- CLI: resty -I /app/src -I /app/lib migrations.lua <up|down|status|seed>
local _M = {}

local ADVISORY_LOCK = 727727727
...
local function lock(pg)
  -- pgmoon LuaJIT'te sayıyı double/numeric gönderir; pg_advisory_lock(bigint) yok
...
function _M.seed(pg, seeds_dir, opts)
  ...
  -- demo_data kosullu (minimal)
  local demo_path = seeds_dir .. "/demo_data.lua"
  local chunk3 = loadfile(demo_path)
  if chunk3 then
    local ok, mod = pcall(chunk3)
    if ok and mod and mod.run then
      local enabled = true
      if mod.enabled then enabled = mod.enabled(env) end
      if enabled then
        local s, serr = pcall(mod.run, q)
        if not s then return nil, "demo_data: " .. tostring(serr) end
        print("seeded demo_data")
      else
        print("seed skipped demo_data (SEED_DEFAULTS false)")
      end
    end
  end
  return true
...
if arg and arg[0] and arg[0]:match("migrations%.lua$") then
  ...
  local db = os.getenv("DB_NAME") or "pgeditor"
  local user = os.getenv("DB_USER") or "pgeditor"
