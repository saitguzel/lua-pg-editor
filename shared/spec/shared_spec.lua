-- bootstrap: shared modulleri preload et (pg-editor)
package.path = package.path .. ";shared/src/?.lua;shared/src/?/init.lua"
package.preload["pg_shared.types"] = function() return dofile("shared/src/types.lua") end
package.preload["pg_shared.validation"] = function() return dofile("shared/src/validation.lua") end
package.preload["pg_shared.protocol"] = function() return dofile("shared/src/protocol.lua") end
...
describe("types", function()
  it("ROLES 2 eleman ve doğru", function()
    assert.same({ "admin", "editor" }, types.ROLES)
  end)
  it("PAGES 17 eleman", function()
    assert.equal(17, #types.PAGES)
    assert.equal("dashboard", types.PAGES[1])
    assert.equal("settings", types.PAGES[17])
  end)
  ...
  it("editor yalnızca editor sayfaları", function()
    assert.is_true(types.default_permission("editor", "dashboard"))
    assert.is_true(types.default_permission("editor", "connections.list"))
    assert.is_true(types.default_permission("editor", "query.execute"))
    assert.is_true(types.default_permission("editor", "table.browser"))
    assert.is_false(types.default_permission("editor", "users.list"))
    assert.is_false(types.default_permission("editor", "rbac.matrix"))
    assert.is_false(types.default_permission("editor", "object.actions"))
  end)
  it("is_locked", function()
    assert.is_true(types.is_locked("admin", "rbac.matrix"))
    assert.is_false(types.is_locked("editor", "rbac.matrix"))
  end)
  it("AUDIT_ACTIONS 27 eleman", function()
    assert.equal(27, #types.AUDIT_ACTIONS)
  end)
  it("default_matrix 34 hücre ve valid", function()
    local m = types.default_matrix()
    assert.equal(34, #m.permissions)
...
describe("protocol", function()
  ...
  it("CODE_LIST 29 ve tekrarsız", function()
    assert.equal(29, #protocol.CODE_LIST)
    local seen = {}
  ...
  it("error_body şekli", function()
    local err = protocol.new_error("CONNECTION_NOT_FOUND", nil, { id = "123" })
    local body = protocol.error_body(err, "req-123")
    assert.equal("CONNECTION_NOT_FOUND", body.error.code)
...
describe("validation.schemas", function()
  it("connection_create geçer", function()
    local c = v.validate(v.schemas.connection_create, { name = "pg", host = "localhost", port = 5432, database = "mydb", username = "user" })
    assert.is_not_nil(c)
  end)
...
describe("validation.schemas geçerli/geçersiz", function()
  ...
  local cases = {
    ...
    query_execute = { { connection_id = uuid, sql = "SELECT 1" }, { connection_id = "x", sql = "" } },
    user_create = { { email = "a@b.com", password = "Admin123!", role = "editor" },
    ...
    rbac_matrix = { { permissions = { { role = "admin", page_key = "dashboard", can_access = true } } },
                    { permissions = {} } },
    pagination = { { page = "2" }, { per_page = "500" } },
    user_list_query = { { is_active = "true" }, { is_active = "evet" } },
    audit_query = { { user_id = uuid }, { user_id = "x" } },
    connection_create = { { name = "test", host = "localhost", port = 5432, database = "mydb", username = "user" }, { name = "" } },
  }
...
describe("saflık", function()
  it("global sızıntı yok", function()
    ...
    require("pg_shared.types")
    require("pg_shared.protocol")
...
describe("pg_shared host/sql_identifier", function()
  it("host geçerli", function()
    assert.is_not_nil(v.validate(v.schema({ h = v.host() }), { h = "localhost" }))
    assert.is_not_nil(v.validate(v.schema({ h = v.host() }), { h = "/var/run/postgresql" }))
  end)
  it("sql_identifier rezerve red", function()
    local _, e = v.validate(v.schema({ t = v.sql_identifier() }), { t = "select" })
    assert.is_not_nil(e.t)
  end)
end)
