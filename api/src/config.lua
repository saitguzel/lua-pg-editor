-- Uygulama konfigürasyonu: env'den okunur, doğrulanır, immutable (pg-editor)
...
local SPEC = {
  ...
  DB_NAME = { type = "string", default = "pgeditor", required = true },
  DB_USER = { type = "string", default = "pgeditor", required = true },
  ...
  JWT_ISSUER = { type = "string", default = "pg-api" },
  ...
  SMTP_FROM = { type = "email", default = "no-reply@pgeditor.local" },
  ...
  TARGET_POOL_MAX = { type = "int", default = 32, min = 1, max = 100 },
  ...
  COMPLETION_CACHE_TTL = { type = "int", default = 300, min = 1 },
}
...
local function cross_validate(c)
  ...
  if c.app and c.app.env == "production" then
    ...
    if c.encryption_key == "dev-encryption-key-32-bytes-long!!" then
      errs[#errs + 1] = "ENCRYPTION_KEY uretimde ornek deger olamaz"
    end
  end
  -- ENCRYPTION_KEY base64 32 byte kontrolu (opsiyonel, ngx varsa)
  if c.encryption_key and #c.encryption_key < 32 then
    errs[#errs + 1] = "ENCRYPTION_KEY en az 32 karakter"
  end
  -- QUERY limit tutarliligi
  if c.query and c.query.row_limit_default and c.query.row_limit_max then
    if c.query.row_limit_default > c.query.row_limit_max then
      errs[#errs + 1] = "QUERY_ROW_LIMIT_DEFAULT, QUERY_ROW_LIMIT_MAX'tan buyuk olamaz"
    end
  end
  if c.target and c.target.pool_size and c.target.pool_max then
...
function _M.load(getenv)
  ...
  c.encryption_key = values.ENCRYPTION_KEY
  c.target = {
    pool_size = values.TARGET_POOL_SIZE,
    pool_max = values.TARGET_POOL_MAX,
  }
  c.query = {
...
function _M.redacted(c)
  ...
  for k, v in pairs(c) do
    ...
    elseif k == "DB_PASSWORD" or k == "JWT_SECRET" or k == "SMTP_PASSWORD" or k == "ENCRYPTION_KEY" then
      out[k] = "***"
    elseif k == "encryption_key" then
      out[k] = "***"
    else
      out[k] = v
    end
