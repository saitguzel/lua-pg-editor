-- Parametreli sorgu katmanı: pgmoon extended protocol, $n yer tutucular (pg-editor)
...
function _M.builder()
  ...
  function self:build_where()
    local where = #self.clauses > 0 and ("WHERE " .. table.concat(self.clauses, " AND ")) or ""
    return where, self.params
  end
  function self:build()
    return self:build_where()
  end
  function self:param(value)
    self.params[#self.params + 1] = value
    return #self.params
  end
  return self
