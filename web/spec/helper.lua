...
-- shared modüller bundle'da "pg_shared.<ad>" olarak yer alır; testte ../shared/src'ye eşlenir
table.insert(package.searchers, 2, function(name)
  local short = name:match("^pg_shared%.(.+)$")
...
fake.dom = {
  ...
  viewport = function() return { 1280, 800 } end,
  focusMove = function(sel, delta) fake.focus_moved = { sel, delta } end,
  _size = function()
    local n = 0
...
fake.http = {
  ...
  download = function(url, token, filename, cb, body)
    fake.calls[#fake.calls + 1] = { method = "DOWNLOAD", url = url, token = token, filename = filename, body = body }
    if cb then cb(fake.download_error, fake.download_error_body) end
  end,
}
...
local function decode(s)
  ...
  value = function(in_array)
    ...
    elseif s:find("^null", pos) then pos = pos + 4 return in_array and "\1pg:null\1" or nil -- glue.js ile ayni
    else
...
fake.json = { decode = decode, encode = function(v) return require("json").encode(v) end,
  pretty = function(s) return s end }
